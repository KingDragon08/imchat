...
class NiuniuService1 {
     ...
     * 
     * Redis::get(groupOwner:{{groupId}}->owner name)
     * 
     ...
     */
    public static function create ($groupId, string $banker, int $jifen) {
        $groupInfo = GroupsModel::select('*')->where('groupId', $groupId)->where('type', 'niuniu')->get()->toArray();
        // 不是牛牛群
        if (empty($groupInfo)) {
        ...
        // 一个群只能有一场进行中的游戏
        $gameExist = NiuniuModel::select('*')->where('groupId', $groupId)->where('status', 0)->count();
        if ($gameExist > 0) {
            throw new Exception("一个群只能有一场进行中的游戏");
        }

        // 删除上一场游戏的下注信息
        $joinersKey = self::GAME_NAME . $groupId;
        ...
        try {
            ...
            // 缓存的游戏信息
            $data = Cache::get(self::GAME_NAME . $groupId);
            $data = json_decode($data, true);

            $currentBankerModel = UserModel::select('*')->where('username', $banker)->first();
            $groupOwner = Redis::get('groupOwner:' . $groupId); // 取群主
            ...
            Cache::put(self::GAME_NAME . $groupId, json_encode($cache), self::CACHE_TIME);
     ...
     * @return [type]          [description]
     */
    public static function bet($userId, $groupId, int $bet, string $type) {
        $game = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($game == -1) {
        ...
        // 获取游戏配置
        $config = GroupsModel::select('cfg')->where('groupId', $groupId)->first()->cfg;
     ...
     * @return [type]          [description]
     */
    public static function end ($groupId) {
        $gameInfo = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($gameInfo == -1) {
            throw new Exception("游戏不存在");
        }
        ...
        $gameInfo['bonusId'] = 0;
        // 写回游戏信息
        Cache::put(self::GAME_NAME . $groupId, json_encode($gameInfo), self::CACHE_TIME);
        // 构造返回字符串
        $strs = [];
        $strs[] = '⭐⭐停止下注⭐⭐';
     ...
     * @return [type]          [description]
     */
    public static function sendBonus($groupId) {
        $gameInfo = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($gameInfo == -1) {
            throw new Exception("游戏不存在");
        }
        ...
        if ($config['bonusFee'] == 'banker') {
            ...
            $bankerInfo->save();
            $bonusId = UserService::groupBonus($bankerInfo['id'], $groupId, 'shouqi', $gameInfo['amount'], $gameInfo['number'], '牛牛红包');
            // 扣掉庄的积分
            $gameInfo['jifen'] -= $gameInfo['amount'];
        }
        // 包费扣除方式->扣群主
        if ($config['bonusFee'] == 'group') {
            $bonusId = UserService::groupBonus($groupOwnerModel->id, $groupId, 'shouqi', $gameInfo['amount'], $gameInfo['number'], '牛牛红包');
        }
        // 包费扣除方式->自认包费
        if ($config['bonusFee'] == 'every') {
             ...
             */
            // 上线后需要设置一个专门用于扣钱的账号
            $bonusId = UserService::groupBonus(10, $groupId, 'shouqi', $gameInfo['amount'], $gameInfo['number'], '牛牛红包');
            /**
             * 非常重要
             */
        }
        ...
        $game = NiuniuModel::select('*')->where('groupId', $groupId)->where('status', 0)->first();
     ...
     */
    public static function setConfig ($groupId, string $config) {
        // 游戏中不允许设置规则
        $gameExist = NiuniuModel::select('*')->where('groupId', $groupId)->where('status', 0)->count();
        if ($gameExist > 0) {
     ...
     * @return [type]          [description]
     */
    public static function getConfig ($groupId) {
        $model = GroupsModel::select('cfg')->where('groupId', $groupId)->first();
     ...
     * @return [type]          [description]
     */
    public static function openBonus($groupId, int $bonusId, $userId) {
        $game = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($game == -1) {
            throw new Exception("游戏不存在");
        }
     ...
     * @return [type]          [description]
     */
    public static function result ($groupId) {
        $timestamp = time();
        $game = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($game == -1) {
            throw new Exception("游戏不存在");
        ...
        try {
            ...
            // 写游戏信息
            $game['bonusId'] = -1;
            Cache::put(self::GAME_NAME . $groupId, json_encode($game), self::CACHE_TIME);
            // 写群主信息
            $groupOwner = Redis::get('groupOwner:' . $groupId); // 取群主
            $groupOwnerModel = UserModel::select('*')->where('username', $groupOwner)->first();
     ...
     * @return [type] [description]
     */
    public static function bang($groupId) {
        // 获取群组成员
        $users = EaseService::groupInfo($groupId);
        $userIds = [$users['owner']['id']];
        $userIds = array_merge($userIds, array_column($users['members'], 'id'));
        $data = UserModel::select('nickname', 'bonus')->whereIn('id', $userIds)->where([['bonus', '>', 0]])->orderBy('bonus', 'desc')->limit(1000)->get()->toArray();
        $strs = [];
        $strs[] = '====🎩土豪排行榜🎩====';
        $strs[] = '人数:👤' . count($data);
    ...
    public static function list() {
        ...
        $data = EaseService::getGroupsInfo(array_column($groups, 'groupId'));
        $data = array_column($data, 'name', 'id');
        for ($i=0; $i<count($groups); $i++) {
            $groups[$i]['nickname'] = $data[$groups[$i]['groupId']];
            $groups[$i]['username'] = $groups[$i]['groupId'];
        }
        return $groups;
     ...
     * @return [type]          [description]
     */
    public static function reset($groupId) {
        $game = Cache::get(self::GAME_NAME . $groupId, -1);
        if ($game == -1) {
            throw new Exception("游戏不存在");
        }
        ...
        if ($game['bonusId'] < 1) {
            ...
            Redis::expire($joinersKey, 0);
            // 结束本局游戏
            $niuniuModel = NiuniuModel::select('*')->where('groupId', $groupId)->first();
